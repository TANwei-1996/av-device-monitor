...
import type { RecordingConfig, RecordingStatus, RecordingResult } from '../../shared/types';
...
class AudioRecorder {
  ...
  private writeWavHeader(config: RecordingConfig, dataSize: number = 0): Buffer {
  ...
  async start(config: RecordingConfig): Promise<string> {
    ...
    logger.info('Recorder', 'Started recording', { filepath: this.filepath, config });
    return this.filepath;
  }

  writeSamples(samples: Float32Array) {
    if (!this.isRecording || this.isPaused || !this.writeStream || !this.config) return;
...
export function registerRecorderIPC() {
  ipcMain.handle(IPC_CHANNELS.START_RECORDING, async (_event, config: RecordingConfig) => {
  ...
  ipcMain.handle(IPC_CHANNELS.GET_RECORDING_STATUS, async () => {
